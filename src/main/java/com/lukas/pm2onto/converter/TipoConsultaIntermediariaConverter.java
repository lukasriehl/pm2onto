 ...
 * @author lukas
 */
@FacesConverter("tipoConsultaIntermediariaConverter")
public class TipoConsultaIntermediariaConverter implements Converter {
    ...
    public Object getAsObject(FacesContext context, UIComponent component, String value) {
        if (value != null && value.trim().length() > 0) {
            ...
            try {
                List<TipoConsulta> tipoConsultaList = bo.retornaTiposConsultasIntermediarias();
                ...
                throw new ConverterException(new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erro", "Erro ao realizar conversão"
                        + "de tipos de consultas avançadas."));
            }
        } else {
            return null;
        }
